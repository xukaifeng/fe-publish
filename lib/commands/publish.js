    ...
    if (checkDeployConfigExists()) {
      ...
      if (env) {
        ...
        if (answers.confirm) {
          ...
          succeed(
            `恭喜您，${underline(projectName)}项目已在${underline(envConfig.name)}部署成功 耗时${
        ...
        if (answers.confirm) {
          for (const env of cluster) {
            ...
            succeed(
              `恭喜您，${underline(projectName)}项目已在${underline(envConfig.name)}部署成功`
            );
          }

          succeed(
            `恭喜您，${underline(projectName)}项目已在${underline("集群环境")}部署成功 耗时${
              (new Date().getTime() - currentTime) / 1000
            }s\n`
          );
        } else {
        ...
        error("请使用 deploy-cli-service -mode 指定部署环境或在配置文件中指定 cluster（集群）地址");
        process.exit(1);
      }
    } else {
      error("deploy.config.js 文件不存，请使用 deploy-cli-service init 命令创建");
      process.exit(1);
    }
  },
};
